...
parse_index_expr <- function(
  ...
  var_len <- paste0(var_struct, ".", LEN_TYPE)
  ...
  if (length(parsed_args) == 1) {
    index_offset_expr <- paste0("(int) ((int) (", parsed_args[1], " - 1) % ", var_len, ")")
  }
  ...
  # offset to 3rd through 4th column
  else if (parsed_args[1] == NULL_ARG) {
    ...
    col_offset <- paste0("((", col_eval, " - 1)", " * ", var_rows, ")")
  ...
  # offset to 3rd through 4th row
    ...
    index_offset_expr <- paste0("(int) ((int) (", col_offset, " + ", row_eval, " - 1) % ", var_len, ")")
  }
  ...
  # Offset to 3rd through 4th row for 5th to 7th columns
    ...
    # The expression to take the column and row offsets and calculate the index
    index_offset_expr <- paste("(int) ((int) (", col_offset, " + ", row_eval, "- 1) % ", var_len, ")")
  }
