...
write_kernel <- function(expr_ls, var_names) {
  ...
  g_loop_count <- 0
  g_loop_exprs <- rep("0", MAX_LOOPS)
  g_loop_env <- envrionment()
  ...
  iteration_loop_lines <- write_iter_loop_lens(kernel_lines[stop_index:length(kernel_lines)],
                                               g_loop_exprs, g_loop_count)

  # Combine all lines to write to output
  lines_to_write <- c(lines_to_write, indent_lines("}", DEFAULT_DEPTH - 1),
                      iteration_loop_lines)
...
write_iter_loop_lens <- function(lines_to_edit, g_loop_exprs, g_loop_count) {
  # TODO
}
...
write_for_loop <- function(args, var_names, depth, index) {
  ...
  save_iter_loop_expr(args[2], var_names)
...
save_iter_loop_expr <- function(args[2], var_names) {
  update_expr <- substitute(g_loop_exprs[g_loop_count + 1] <- parse_expr_len(args[2], var_names))
  update_count <- substitute(g_loop_count <- g_loop_count + 1)
  eval(update_expr, envir = g_loop_env)
  eval(update_count, envir = g_loop_env)
}
...
parse_expr_len <- function(expr, var_names) {
  # TODO
}
